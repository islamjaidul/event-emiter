...
**Run the full suite after every change.** If a test fails, fix the source before moving on — never skip or weaken assertions. Use `/test` to have Claude run, diagnose, and fix failing tests automatically.
...
- Test patterns and Jest config → `.claude/commands/test.md` (has concrete examples for every test file).
